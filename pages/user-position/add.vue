...
const { generateUserPositionID, insertUserPosition, } = useUserPosition();
...
onMounted(async () => {
  try {
    const id = await generateUserPositionID()
    user_position.value.user_position_id = id
  } catch (e) {
    console.log(e)
  }
})
...
async function onSubmit() {
  ...
  try {
    ...
    await insertUserPosition(user_position.value);
...
function validateForm(): boolean {
  ...
  if (!(user_position_id && user_position_name)) {
        ...
        <v-col cols="12" md="12">
          <v-row>
            <v-col cols="12" sm="6" lg="3">
              <v-label class="mb-2">รหัสตำเเหน่ง<span class="text-error ml-1">*</span></v-label>
              <v-text-field v-model="user_position.user_position_id" density="compact" :rules="[rules.required]"
