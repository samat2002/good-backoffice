...
const { getPortfolioBy, deletePortfolioBy } = usePortfolio();
const { $auth } = useNuxtApp()
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('portfolio')
...
const headers: Headers = [
  ...
  { title: "รหัสประสบการณ์", key: "portfolio_id", sortable: false, width: 200, },
  { title: "ชื่อประสบการณ์", key: "portfolio_name", sortable: false },
];
const columns: any[] = [
  { value: "portfolio_id", title: "รหัสประสบการณ์", },
  { value: "portfolio_name", title: "ชื่อประสบการณ์", },
];
...
} = await useAsyncData('portfolios', () => getPortfolioBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
...
const onDelete = (id: string) => Swal.fire({
  ...
  try {
    ...
    await deletePortfolioBy({ portfolio_id: id })
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    ...
    <v-card-item>
      <v-row class="mb-4">
        <v-col class="d-flex flex-wrap gap-2 align-center py-2" cols="12" md="4">
          <template v-if="permission_add">
            <v-btn elevation="3" color="primary" to="/portfolio/add">เพิ่มรายการใหม่</v-btn>
          </template>
        </v-col>
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              ...
              @click:append-inner="refreshPortfolios()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingPortfolios" :headers="headers" :items="portfolios?.docs ?? []"
        :items-length="portfolios?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/portfolio/detail', query: { id: item.portfolio_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/portfolio/update', query: { id: item.portfolio_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.portfolio_id)">
                  <v-list-item-title>
