...
const { getConTactBy, deleteConTact } = useConTact();
const { $auth } = useNuxtApp()
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('contact')
...
const headers: Headers = [
  ...
  { title: "รหัสการติดต่อ", key: "contact_id", sortable: false, width: 200, },
  { title: "รายชื่อผู้ติดต่อ", key: "contact_name", sortable: false },
];
const columns: any[] = [
  { value: "contact_id", title: "รหัสการติดต่อ", },
  { value: "contact_name", title: "รายชื่อผู้ติดต่อ", },
];
...
} = await useAsyncData('contact', () => getConTactBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
...
const onDelete = (id: string) => Swal.fire({
  ...
  try {
    ...
    await deleteConTact({ contact_id: id })
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    <v-card-title>ติดต่อเรา</v-card-title>
    <v-card-item>
      <v-row class="mb-4">
        <v-col class="d-flex flex-wrap gap-2 align-center py-2" cols="12" md="4">
          <!-- <template v-if="permission_add">
            <v-btn elevation="3" color="primary" to="/contact/add">เพิ่มรายการใหม่</v-btn>
          </template> -->
        </v-col>
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              ...
              @click:append-inner="refreshConTact()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingConTact" :headers="headers" :items="contacts?.docs ?? []"
        :items-length="contacts?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/contact/detail', query: { id: item.contact_id } }">
                <v-list-item-title>
                  <v-icon> mdi-chat-processing-outline</v-icon> ดูรายละเอียด
                </v-list-item-title>
              </v-list-item>
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.contact_id)">
                  <v-list-item-title>
