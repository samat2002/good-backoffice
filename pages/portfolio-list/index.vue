...
const { getPortfolioListBy, deletePortfolioList } = usePortfolioList();
const { $auth } = useNuxtApp()
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('portfolio-list')
...
const headers: Headers = [
  ...
  { title: "รูป", key: "portfolio_list_img", sortable: false, width: 80, align: 'center' },
  { title: "รหัสรูปทักษะ", key: "portfolio_list_id", sortable: false, width: 200, },
  { title: "ชื่อ", key: "portfolio_list_name", sortable: false },
  { title: "รายละเอียด", key: "portfolio_list_detail", sortable: false },
];
const columns: any[] = [
  { value: "portfolio_list_id", title: "รหัสรูปทักษะ", },
  { value: "portfolio_list_name", title: "ชื่อ", },
  { value: "portfolio_list_detail", title: "รายละเอียด", },
];
...
const {
  data: portfolio_lists,
  pending: pendingPortfolioList,
  refresh: refreshPortfolioList,
} = await useAsyncData('portfolio_lists', () => getPortfolioListBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
...
const onDelete = (id: string) => Swal.fire({
  ...
  try {
    ...
    await deletePortfolioList({ portfolio_list_id: id })
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    <v-card-title>รูปทักษะ</v-card-title>
    <v-card-item>
      <v-row class="mb-4">
        <v-col class="d-flex flex-wrap gap-2 align-center py-2" cols="12" md="4">
          <template v-if="permission_add">
            <v-btn elevation="3" color="primary" to="/portfolio-list/add">เพิ่มรายการใหม่</v-btn>
          </template>
        </v-col>
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              ...
              @click:append-inner="refreshPortfolioList()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingPortfolioList" :headers="headers" :items="portfolio_lists?.docs ?? []"
        :items-length="portfolio_lists?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
        ...
        <template v-slot:[`item.portfolio_list_img`]="{ item }">
          <v-avatar>
            <v-img v-if="item.portfolio_list_img" class="mx-auto"
              :src="`${publicCtx.apiBaseUrl}${item.portfolio_list_img}`" width="50px" height="50px" cover alt="Avatar">
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/portfolio-list/detail', query: { id: item.portfolio_list_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/portfolio-list/update', query: { id: item.portfolio_list_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.portfolio_list_id)">
                  <v-list-item-title>
