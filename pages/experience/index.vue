...
const { getExperienceBy, deleteExperienceBy } = useExperience();
const { $auth } = useNuxtApp()
const { permission_add, permission_delete, permission_edit } = $auth.getPermission('experience')
...
const headers: Headers = [
  ...
  { title: "รหัสประสบการณ์", key: "experience_id", sortable: false, width: 200, },
  { title: "ชื่อประสบการณ์", key: "experience_name", sortable: false },
];
const columns: any[] = [
  { value: "experience_id", title: "รหัสประสบการณ์", },
  { value: "experience_name", title: "ชื่อประสบการณ์", },
];
...
} = await useAsyncData('experiences', () => getExperienceBy({ pagination: pagination.value, search: search.value }), { watch: [pagination.value] })
...
const onDelete = (id: string) => Swal.fire({
  ...
  try {
    ...
    await deleteExperienceBy({ experience_id: id })
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    <v-card-title>ประสบการณ์</v-card-title>
    <v-card-item>
      <v-row class="mb-4">
        <v-col class="d-flex flex-wrap gap-2 align-center py-2" cols="12" md="4">
          <template v-if="permission_add">
            <v-btn elevation="3" color="primary" to="/experience/add">เพิ่มรายการใหม่</v-btn>
          </template>
        </v-col>
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              ...
              @click:append-inner="refreshExperiences()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingExperiences" :headers="headers" :items="experiences?.docs ?? []"
        :items-length="experiences?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/experience/detail', query: { id: item.experience_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/experience/update', query: { id: item.experience_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.experience_id)">
                  <v-list-item-title>
